...
const Message = ({ chats, typings }) => {
  ...
  return (
    <div className="chat-warp m-scrollbar force-overflow" ref={messagesRef}>
      {
        typings.length !== 0
        && <p className="typing">{typings.join(', ')}님이 채팅을 입력하고 있습니다.</p>
      }
          ...
          chats.length !== 0 && (
            chats.map(message =>
              message.user.id === currentUser.uid ? (
                  <li className="chat-content chat-right animated fadeIn fast" key={message.key}>
                ...
                :
                (
                  <li className="chat-content chat-left animated fadeIn fast" key={message.key}>
